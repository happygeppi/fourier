...
class DJ {
  ...
  round(_n, _d = 0) {
    return Math.round(_n * 10**_d) / 10**_d;
  }
  ...
  toHex(_n) {
    ...
    let _digits = [];
    let _power = 0;
    while (_n > 16 ** _power - 1) _power++;
    _power--;

    while (_power >= 0) {
      let _digit = 0;
      while (_n >= _digit * 16 ** _power) _digit++;
      _digit--;
      _digits.push(_digit);
      _n -= _digit * 16 ** _power;
      _power--;
  ...
  toBase(_b, _n) {
    ...
    let _digits = [];
    let _power = 0;
    while (_n > _b ** _power - 1) _power++;
    _power--;

    while (_power >= 0) {
      let _digit = 0;
      while (_n >= _digit * _b ** _power) _digit++;
      _digit--;
      _digits.push(_digit);
      _n -= _digit * _b ** _power;
      _power--;
  ...
  getAngle() {
    ...
    if (args.length == 1) {
      ...
      A = this.vector.new(args[0], args[1]);
      B = this.vector.new(args[2], args[3]);
    }
    return this.atan(B.y - A.y, B.x - A.x);
  }
  copyArray(_arr) {
    let _newArr = [];
    for (let _elem of _arr) _newArr.push(_elem);
    return _newArr;
  }
  ...
  stopTimer() {
    this.timeB = new Date();
    return (this.stopwatch = this.timeBetween(this.timeA, this.timeB));
  }
  timeBetween(_A, _B) {
  ...
  keyDown() {
    const args = arguments.copy().toArray();
    if (args.length === 0 && this.keysDown.length > 0) return true;
  ...
  doIf(_cdts, _func) {
    ...
    if (typeof _cdts == "object") {
      if (_cdts[0] == 1) return _func();
  
      _cdts.forEach((_cdt) => {
        if ((_cdt == CLICK && this.mouse.down) || this.keyPressed(_cdt)) _func();
      });
    } else if (typeof _cdts == "boolean") {
      if (_cdts) return _func();
    }
  }
  checkFramerate = () => (this.checkFR = true);
  checkRunning() {
    this.doIf(this.stopCdts, this.cancel);
    this.doIf(this.updateCdts, this.addUpdater);
  }
  handleInput() {
  ...
  protoFuncs() {
    ...
    Array.prototype.copy = function () {
      let _newArr = [];
      for (let _elem of this) _newArr.push(_elem);
      return _newArr;
    };
    Array.prototype.random = function () {
      return this[dj.floor(dj.random(this.length))];
    };
    Array.prototype.hexToDec = function () {
      let _num = 0;
      this.forEach((_digit, _i) => {
        const _power = this.length - _i - 1;
        _num += _digit * 16 ** _power;
      });
      return _num;
    };
    Object.prototype.copy = function () {
      let _copy = new this.constructor();
      for (let _attr in this)
        if (this.hasOwnProperty(_attr)) _copy[_attr] = this[_attr];
      return _copy;
    };
    Object.prototype.toArray = function () {
      let _newArr = [];
      for (let _attr in this)
        if (this.hasOwnProperty(_attr)) _newArr.push(this[_attr]);
      return _newArr;
    };
    // toVec()
  }
...
function BackgroundDraw() {
  ...
  dj.handleInput();
  if (dj.framecount % 60 === 0 && dj.canvas !== undefined)
